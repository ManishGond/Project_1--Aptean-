page 50106 "Requests to Approve Card PO"
{
    ApplicationArea = Suite;
    Caption = 'Custom Req PO Approval Card';
    Editable = false;
    PageType = List;
        ...
        area(processing)
        {
            action(Approve)
            {
                ...
                var
                    ...
                    ApprovalsMgmt: Codeunit "Approvals Mgmt.";
                    ApprovalRequest: Codeunit PublisherEventPR4;
                    POTable: Record "Purchase Header";
                    PLTable: Record "Purchase Line";
                ...
                begin
                    ...
                    POTable.SetRange("No.", ApprovalEntry."Document No.");
                    POTable.FindSet();
                    REPEAT
                        POTable.Status := POTable.Status::Released;
                        POTable.Modify();
                    UNTIL POTable.Next() = 0;

                    ApprovalRequest.StatusChange2(ApprovalEntry);
                end;
            }
            action(Reject)
            {
